...
<template>
	<div 	v-if="show"
				:class="'loading-bar to-' + direction + full"
		...
		watch:{
			...
			progress(val,old){
				...
				if(old == 0 && val > 0){
					...
					eventBus.$emit('loading-bar:started');
				}

				if(val > 1 && val < 100){
					// Callback Event when it's loading
					eventBus.$emit('loading-bar:loading');
				}
						...
						this.error = false;
							...
							setTimeout(() => {
								...
								setTimeout(() => {
									...
									eventBus.$emit('loading-bar:loaded');
								});
			...
			error(val,old){
				...
				eventBus.$emit('loading-bar:error');
			}
